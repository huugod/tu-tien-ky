...
router.get('/metadata/all', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const bonusTypes = [
            ...
            'hp_add', 'atk_add', 'def_add', 'hp_mul', 'atk_mul', 'def_mul', 'speed_add', 
            ...
            'atk_mul_buff', 'def_mul_buff', 'hp_mul_buff', 'breakthrough_chance_add_buff',
        ];
...
router.put('/players/:name', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        if (updates.inventory || updates.equipment) {
            ...
            for (const item of equippedItemsFromClient) {
                ...
                if (item.instance_id) { // Update existing item
                    await conn.query(
                        "UPDATE player_equipment SET equipment_id = ?, is_equipped = TRUE, slot = ? WHERE instance_id = ?",
                        [item.equipment_id, staticData.slot, item.instance_id]
                    );
                } else { // Add new equipped item
                    await conn.query(
                        "INSERT INTO player_equipment (player_name, equipment_id, is_equipped, slot) VALUES (?, ?, TRUE, ?)",
                        [name, item.equipment_id, staticData.slot]
                    );
                }
            }
            ...
            for (const item of inventoryItemsFromClient) {
                if (item.instance_id) { // Update existing item
                    await conn.query(
                        "UPDATE player_equipment SET equipment_id = ?, is_equipped = FALSE, slot = NULL WHERE instance_id = ?",
                        [item.equipment_id, item.instance_id]
                    );
                } else { // Add new inventory item
                    await conn.query(
                        "INSERT INTO player_equipment (player_name, equipment_id, is_equipped, slot) VALUES (?, ?, FALSE, NULL)",
        ...
        if (columns.length > 0) {
            ...
            // FIX: Pass objects directly to the driver; remove manual JSON.stringify
...
const createCrudEndpoints = (tableName, primaryKey = 'id') => {
    ...
    router.post(`/${tableName}`, authenticateAdmin, async (req, res) => {
        ...
        try {
            ...
            // FIX: Pass objects directly to driver, remove manual JSON.stringify
            const values = columns.map(col => formatDateTimeForDB(itemData[col]));
    ...
    router.put(`/${tableName}/:key`, authenticateAdmin, async (req, res) => {
        ...
        try {
            ...
            const setClause = columns.map(col => `\`${col}\` = ?`).join(', ');
            // FIX: Pass objects directly to driver, remove manual JSON.stringify
            const values = columns.map(col => formatDateTimeForDB(updates[col]));
